...
type PdfContent = PdfParagraph | PdfPageBreak | PdfTable | PdfText;
...
export interface PdfOptions {
   ...
   * @default "Helvetica"
   */
  font?: StandardFontType | CustomFont;
  /**
   * Page size.
...
export async function mdastToPdf(
  ...
  const renderInlines = (
    ...
    for (let i = 0; i < texts.length; i++) {
      ...
      let font = defaultFont.normal;
      if (style.bold && style.italic && defaultFont.bolditalic) {
        font = defaultFont.bolditalic;
      } else if (style.bold && defaultFont.bold) {
        font = defaultFont.bold;
      } else if (style.italic && defaultFont.italic) {
